...
await mkdir(join(output, 'review'), { recursive: true })
await mkdir(join(output, 'reviews-admin'), { recursive: true })
await mkdir(join(output, 'booking'), { recursive: true })
...
await Promise.all([
  ...
  cp(join(root, 'review.html'), join(output, 'review', 'index.html')),
  cp(join(root, 'reviews-admin.html'), join(output, 'reviews-admin', 'index.html')),
  cp(join(root, 'booking.html'), join(output, 'booking', 'index.html')),
